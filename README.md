...
observed. The gate fails quiet: it does not reopen merely because time elapsed
while microphone energy remains above the quiet threshold. This prevents
Reachy's own speech from becoming a new user turn without mutating server
conversation history. Spoken barge-in during playback is therefore deferred
until an acoustic echo-cancellation path is available; local playback controls
can still stop the current output.
...
| `REACHY_MINI_APP_TIMEOUT_MINUTES` | Minutes of inactivity before Reachy runs its sleep movement, disables its motors, and stops the app when both succeed. Defaults to `1440` (one day); set to `0` to disable. |
...
| `--no-wobble` | `False` | Disable audio-reactive head wobbling before wake-up and for the session. |
| `--diagnostic-antenna-expression` | `False` | Restore idle antenna expression for supervised diagnosis only. Do not use unsupervised or in production. |
| `--ui` | `False` | Serve the web UI at http://127.0.0.1:7860/, in addition to console mode. |
| `--robot-name` | `None` | Optional. Connect to a specific robot by name when running multiple daemons on the same subnet. See [Multiple robots on the same subnet](#advanced-features). |
| `--robot-host` | `None` | Optional. Connect directly to this trusted network daemon host instead of trying localhost first. |
...
| `go_to_sleep` | Run Reachy's sleep movement, disable its motors, and stop the current app only when both succeed. | Core install only. |
