...
When an integration installs the optional search policy and Space-revision gate, the official search tool is handled out of band: policy approval happens before dispatch, and its query, result, and spoken answer stay outside ordinary tool/transcript sinks. Without those hooks, the preinstalled MCP tool keeps its existing behavior.
